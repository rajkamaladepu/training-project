...
@SlingServletResourceTypes(resourceTypes = { "training/service/demoservice" }, methods = { HttpConstants.METHOD_GET,
		HttpConstants.METHOD_POST }, extensions = "json")
@ServiceDescription("Servlet to demo Service")
public class ServiceDemoServlet extends SlingAllMethodsServlet {
	private static final long serialVersionUID = 1L;
	private static final Logger logger = LoggerFactory.getLogger(GetJSONServlet.class);
	
	@Reference(target = "(component.name=DemoServiceImplA)")
	DemoService demoServiceA;

	@Reference(target = "(component.name=DemoServiceImplB)")
	DemoService demoServiceB;
	...
	protected void doGet(SlingHttpServletRequest request, SlingHttpServletResponse response)
		...
		test.addProperty("My Name from Service A", demoServiceA.getName());
		test.addProperty("My Name from Service B", demoServiceB.getName());
		tilesArray.add(test);
